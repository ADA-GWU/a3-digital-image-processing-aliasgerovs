...
def apply_mean_filter(image_path):
    image = cv2.imread(image_path, 0)
    kernel_size = (2, 2)
...
def apply_unsharp_filter(image_path):
    image = cv2.imread(image_path, 0)
...
image_path = 'noisy/chemical/inchi4.png'
image = cv2.imread(image_path, 0)
morphological = apply_morphological_operations(image_path)

mean_filtered_image = apply_mean_filter(image_path)
median_filtered_image = apply_median_filter(image_path)
gaussian_smoothed_image = apply_gaussian_smoothing(image_path)
bilateral_filtered_image = apply_bilateral_filter(image_path)

conservative_smoothed_image = apply_conservative_smoothing(image)
low_pass_filtered_image = apply_low_pass_filter(image_path)
high_pass_filtered_image = apply_high_pass_filter(image_path)
laplacian_filtered_image = apply_laplacian_filter(image_path)
unsharp_image = apply_unsharp_filter(image_path)

titles = ['Original Image', 'Morphological', 'Mean Filter', 'Median Filter', 'Gaussian Smoothing',
